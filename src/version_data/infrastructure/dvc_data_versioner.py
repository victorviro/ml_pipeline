    ...
    def version_data(self, data_file_path: str, data_version: float) -> dict:
        ...
        try:
            relative_data_file_path = os.path.relpath(path=data_file_path,
                                                      start=os.getcwd())
            output = subprocess.run(["dvc", "add", relative_data_file_path],
                                    capture_output=True, check=True)
            logger.info('Tracked the dataset in the DVC repository using subprocess. '
            ...
            msg = ('Error tracking the dataset in the DVC repository using subprocess. '
        ...
        try:
            ...
            msg = (f'Error getting git repository using GitPython. There is no .git file '
        ...
        if diff_output:
            ...
            try:
                ...
                msg = ('Error tracking or committing the metadata files of the dataset '
            ...
            try:
                ...
                logger.info(f'Pushed metadata files (.dvc) of the dataset in the branch '
                            f'{self.git_branch_name}. Commit hash: {commit_hash}')

            except ValueError as err:
                message = (f'Error getting remote named {self.git_remote_name} of the git'
                           f' repo. Error: {err}')
                logger.error(message)
                raise ValueError(message)
            except GitCommandError as err:
                msg = ('Error pushing commited metadata files (.dvc) of the dataset in '
