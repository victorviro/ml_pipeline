...
class ValidateModel:
    ...
    def execute(self, data_file_path: str, model_path: str, transformer_path: str):
        ...
        if not os.path.exists(transformer_path):
            raise Exception('Path of transformer file does not exist: '
                            f'"{transformer_path}"')
        # Load the dataset, the model, and the transformer
        ...
        transformer = self.model_file_loader.load_data(file_path=transformer_path)
        # Validate the model
        self.model_validator.validate_model(data=data, transformer=transformer,
                                            model=model)
